...
def alloydb_nl_to_sql(query: str) -> str:
    ...
    try:
        ...
        cursor.execute(
            "SELECT google_ml.predict_row('publishers/google/models/gemini-2.0-flash', json_build_object('contents', json_build_array(json_build_object('role', 'user', 'parts', json_build_array(json_build_object('text', %s))))))",
            (prompt,),
        )
        row = cursor.fetchone()
